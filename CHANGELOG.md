# `react-native-map-direction-with-custom-url` Changelog

## 2.0.0 – 2026-03-26

- Add `useBackendApi` prop to route requests through a custom backend instead of calling Google Maps directly
- Add `backendUrl` prop for the backend endpoint URL (required when `useBackendApi` is true)
- Add `backendAuthToken` prop for optional Bearer token authentication with the backend
- Backend mode sends a POST request (form-encoded) and supports Google Maps Directions API response format
- Backend mode also supports wrapped response formats: `{ data: { routes } }` and `{ getMapsDirectionsResponse: { routes } }`
- Make `apikey` prop optional (not required when using backend mode)
- Fix TypeScript `JSX.Element` namespace error in type definitions
- Update TypeScript definitions to include all new props
- Forked from `react-native-maps-directions` by Bram(us) Van Damme
